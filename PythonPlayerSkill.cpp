...
extern const DWORD c_iSkillIndex_AutoAttack;
...
bool CPythonPlayer::__CheckSkillUsable(DWORD dwSlotIndex)
{
	...
	return pkInstMain->IsAffect(CInstanceBase::AFFECT_CANNOT_USE_SKILL) == 0;
	//return true;
}
...
{
	DWORD dwTargetRange = rkSkillData.GetTargetRange();
	DWORD dwTargetRangeMax = 3500;
	if (rkSkillData.IsNeedBow())
	{
		DWORD dwAddTargetRange = dwTargetRange + 100 * GetStatus(POINT_BOW_DISTANCE);
		if (dwAddTargetRange <= dwTargetRangeMax)
			return dwAddTargetRange;
		else
			return dwTargetRangeMax;
	}
...
bool CPythonPlayer::__ProcessEnemySkillTargetRange(CInstanceBase& rkInstMain, CInstanceBase& rkInstTarget, CPythonSkill::TSkillData& rkSkillData, DWORD dwSkillSlotIndex)
{
	...
	float fSkillTargetRange = static_cast<float>(dwSkillTargetRange);
	if (fSkillTargetRange <= 0.0f)
		return true;

#if defined(__HIT_RANGE_RENEWAL__)
	float fTargetDistance = rkInstMain.GetDistance(&rkInstTarget);
	float fHitRangeDistance = CPythonNonPlayer::Instance().GetMonsterHitRange(rkInstMain.GetRace()) + fTargetDistance;
	float fMaxHitRange = fHitRangeDistance - CPythonNonPlayer::Instance().GetMonsterHitRange(rkInstTarget.GetRace());

	if (fSkillTargetRange < fMaxHitRange)
	{
	...
	if (rkSkillData.IsIgnoreBlock() && "metin2_defensewave" == CPythonBackground::Instance().GetWarpMapName())
		return true;
#endif
...
bool CPythonPlayer::__CanUseSkill(DWORD dwSkillIndex)
{
	CInstanceBase* pkInstMain = NEW_GetMainActorPtr();
	if (!pkInstMain)
		return false;
	...
	if (!pkInstMain->IsMountingHorse())
		return pkInstMain->CanUseSkill(dwSkillIndex);

	if (dwSkillIndex == c_iSkillIndex_Fishing)
		return false;

	// 뉴마운트. 승마스킬레벨 20 미만인 경우, 고급 마운트를 타고 승마 관련 스킬 못 쓰도록 못하도록 하드 코딩... 
	// 나중에 시간 나면 can use skill 체크를 서버에서 해주자...
#if defined(ENABLE_CONQUEROR_LEVEL)
	if (__IsHorseSkill(dwSkillIndex) && (GetSkillGrade(107) < 1 && GetSkillLevel(107) < 20))
#else
	if (__IsHorseSkill(dwSkillIndex) && (GetSkillGrade(109) < 1 && GetSkillLevel(109) < 20))
#endif
	{
		TraceError("__CanUseSkill GetSkillGrade(107) %d", GetSkillGrade(107));
		return false;
	}
...
bool CPythonPlayer::__UseSkill(DWORD dwSlotIndex)
{
	...
	TSkillInstance& rkSkillInst = m_playerStatus.aSkill[dwSlotIndex];
	if (!__CanUseSkill(rkSkillInst.dwIndex))
	{
		if (c_iSkillIndex_Fishing == rkSkillInst.dwIndex)
	...
	if (pSkillData->CanChangeDirection())
	{
		...
		else if (GetPickedActorID(&dwPickedActorID))
		{
...
bool CPythonPlayer::__HasEnoughArrow()
{
	...
	if (CItemManager::Instance().GetItemDataPointer(GetItemIndex(TItemPos(EQUIPMENT, c_Equipment_Slot_Arrow)), &pItemData))
	{
...
void CPythonPlayer::UseGuildSkill(DWORD dwSkillSlotIndex)
{
	...
	TSkillInstance& rkSkillInst = m_playerStatus.aSkill[dwSkillSlotIndex];
	if (!pkInstMain->CanUseSkill(rkSkillInst.dwIndex))
		return;
	...
	if (!pkInstMain->NEW_UseSkill(rkSkillInst.dwIndex, dwMotionIndex, 1, false))
	{
		Tracenf("CPythonPlayer::UseGuildSkill(%d) - pkInstMain->NEW_UseSkill - ERROR", rkSkillInst.dwIndex);
	}

	///////////////////////////////////////////////////////////////////////////////////////////////

	CPythonNetworkStream::Instance().SendGuildUseSkillPacket(rkSkillInst.dwIndex, 0);
	__RunCoolTime(dwSkillSlotIndex);
}
