...
void SNetworkActorData::__copy__(const SNetworkActorData& src)
{
	...
	m_dwEventRace = src.m_dwEventRace;
#endif
	...
	m_wRefineElementAffectType = src.m_wRefineElementAffectType;
#endif
	...
	m_stCountry = src.m_stCountry;
#endif

#if defined(ENABLE_LEFT_SEAT)
	m_bLeftSeat = src.m_bLeftSeat;
#endif
}
...
CInstanceBase* CNetworkActorManager::__AppendCharacterManagerActor(SNetworkActorData& rkNetActorData)
{
...
#if defined(ENABLE_RACE_SWAP)
	kCreateData.m_dwEventRace = rkNetActorData.m_dwEventRace;
#endif
...
#endif
#if defined(ENABLE_REFINE_ELEMENT_SYSTEM)
	kCreateData.m_wRefineElementAffectType = rkNetActorData.m_wRefineElementAffectType;
#endif
	kCreateData.m_isMain = __IsMainActorVID(dwVID);

#if defined(ENABLE_MULTI_LANGUAGE_SYSTEM)
	kCreateData.m_stCountry = rkNetActorData.m_stCountry;
#endif
	...
	if (pOldInstance)
	{
		...
		bool bIsMountingHorse = pOldInstance->IsMountingHorse() ? true : false;
...
#if defined(ENABLE_RACE_SWAP)
	if (kCreateData.m_dwEventRace != 0)
		pNewInstance->SetLoopMotion(CRaceMotionData::NAME_WAIT);
#endif
	...
	if (rkNetActorData.m_dwRace == 33010)
	{
		TPixelPosition kPPosDst;
		kPPosDst.x = kCreateData.m_lPosX;
		kPPosDst.y = kCreateData.m_lPosY;
		kPPosDst.z = 0;
		pNewInstance->PushTCPState(rkNetActorData.m_dwServerSrcTime + dwElapsedTime, kPPosDst, random_range(0, 359), CInstanceBase::FUNC_WAIT, 0);
		pNewInstance->CreateSpecialEffect(CInstanceBase::EFFECT_USE_SNOWFLAKE_STICK);
		return pNewInstance;
	}
#endif
...
void CNetworkActorManager::AppendActor(const SNetworkActorData& c_rkNetActorData)
{
	if (__IsMainActorVID(c_rkNetActorData.m_dwVID))
	{
		...
		if (pMainInstance)
		{
			bool bIsMountingHorse = pMainInstance->IsMountingHorse() ? true : false;
...
void CNetworkActorManager::UpdateActor(const SNetworkUpdateActorData& c_rkNetUpdateActorData)
{
	...
	{
#if defined(ENABLE_LEFT_SEAT)
		pkInstFind->SetLeftSeat(c_rkNetUpdateActorData.m_bLeftSeat);
#endif
#if defined(ENABLE_REFINE_ELEMENT_SYSTEM)
		pkInstFind->SetRefineElement(c_rkNetUpdateActorData.m_wRefineElementAffectType);
#endif
...
#if defined(ENABLE_GUILD_LEADER_GRADE_NAME)
			, c_rkNetUpdateActorData.m_bGuildLeaderGrade
#endif
		);
...
#endif
#if defined(ENABLE_REFINE_ELEMENT_SYSTEM)
	rkNetActorData.m_wRefineElementAffectType = c_rkNetUpdateActorData.m_wRefineElementAffectType;
#endif
...
#if defined(ENABLE_LEFT_SEAT)
	rkNetActorData.m_bLeftSeat = c_rkNetUpdateActorData.m_bLeftSeat;
#endif
}
