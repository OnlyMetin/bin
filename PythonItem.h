...
public:
	...
	bool IsAcceScroll(const DWORD c_dwVnum) const;
#endif

#if defined(ENABLE_CHANGE_LOOK_SYSTEM)
	bool CanAddChangeLookItem(const CItemData* c_pLeftItemData, const CItemData* c_pRightItemData) const;
	bool CanAddChangeLookFreeItem(const DWORD c_dwVnum) const;
	bool IsChangeLookClearScrollItem(const DWORD c_dwVnum) const;
#endif
...
#if defined(ENABLE_SET_ITEM)
public:
	void SetPreName(const int iSetValue, const std::string& c_rstPreName);
	const char* GetPreName(const int iSetValue, const char* c_rstPreName) const;
protected:
	std::unordered_map<int, std::string> m_PreNameMap;
#endif
