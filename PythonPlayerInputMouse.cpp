	...
	switch (m_eReservedMode)
	{
		...
		case MODE_USE_SKILL:
		{
			...
			if (pkInstReserved)
			{
				...
				float fHitRangeDistance = CPythonNonPlayer::Instance().GetMonsterHitRange(pkInstReserved->GetRace()) + fDistance;
				float fMaxHitRange = fHitRangeDistance - CPythonNonPlayer::Instance().GetMonsterHitRange(pkInstReserved->GetRace());

				if (fMaxHitRange < float(m_dwSkillRangeReserved))
				{
