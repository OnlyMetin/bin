...
void CPythonApplication::SetMyShopDecoCameraPosition()
{
	CCamera* pCurrentCamera = CCameraManager::Instance().GetCurrentCamera();
	if (!pCurrentCamera)
		return;
...
void CPythonApplication::SetYutnoriCameraPosition()
{
	CCamera* pCurrentCamera = CCameraManager::Instance().GetCurrentCamera();
	if (!pCurrentCamera)
		return;
	...
	m_pyGraphic.SetPositionCamera(v3Target.x, v3Target.y, v3Target.z + pCurrentCamera->GetTargetHeight(), fDistance, fPitch, fRotation);
}
#endif
