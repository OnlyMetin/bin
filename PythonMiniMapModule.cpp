...
PyObject* miniMapGetAtlasPosition(PyObject* poSelf, PyObject* poArgs)
{
	long lx, ly;
	UI::CWindowManager::Instance().GetMousePosition(lx, ly);

	float fScreenX = static_cast<float>(lx);
	float fScreenY = static_cast<float>(ly);

	float fPosX = 0.0f, fPosY = 0.0f;
	CPythonMiniMap::Instance().GetAtlasPosition(fScreenX, fScreenY, &fPosX, &fPosY);
...
#if defined(ENABLE_ATLAS_GM_GOTO)
		{ "GetAtlasPosition", miniMapGetAtlasPosition, METH_VARARGS },
#endif
