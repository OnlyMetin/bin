...
CPythonGuild::SDragonLairRankingData* CPythonGuild::GetDragonLairRanking(BYTE bType, DWORD dwIndex)
{
	if (dwIndex >= GetDragonLairRankingShowCount(bType))
		return nullptr;

	TDragonLairRankingMap::const_iterator it = m_DragonLairRankingMap.find(bType);
	if (it != m_DragonLairRankingMap.end())
		return it->second.at(dwIndex).get();

	return nullptr;
}

CPythonGuild::SDragonLairRankingData* CPythonGuild::GetMyDragonLairRanking(BYTE bType, DWORD& dwIndex)
{
	dwIndex = 0;

	const DWORD dwGuildID = GetGuildID();
	TDragonLairRankingMap::iterator it = m_DragonLairRankingMap.find(bType);
	if (it != m_DragonLairRankingMap.end())
	{
		for (const TDragonLairRankingVector::value_type& rVector : it->second)
		{
			++dwIndex;

			if (rVector->dwGuildID == dwGuildID)
				return rVector.get();
		}
	}
	return nullptr;
...
size_t CPythonGuild::GetDragonLairRankingShowCount(BYTE bType)
{
	TDragonLairRankingMap::const_iterator it = m_DragonLairRankingMap.find(bType);
	if (it != m_DragonLairRankingMap.end())
...
bool CPythonGuild::CheckDragonLairRanking(BYTE bType)
{
	return m_bCheckDragonLairRanking[bType];
}

bool CPythonGuild::IsDragonLairClearGuild(BYTE bType)
{
	const DWORD dwGuildID = GetGuildID();
	TDragonLairRankingMap::iterator it = m_DragonLairRankingMap.find(bType);
	if (it != m_DragonLairRankingMap.end())
...
void CPythonGuild::ClearDragonLairRanking(BYTE bType)
{
	TDragonLairRankingMap::iterator it = m_DragonLairRankingMap.find(bType);
	if (it != m_DragonLairRankingMap.end())
		it->second.clear();
...
void CPythonGuild::RegisterDragonLairRanking(BYTE bType, DWORD dwGuildID, const char* pszGuildName, BYTE bMemberCount, DWORD dwTime)
{
	m_DragonLairRankingMap[bType].emplace_back(std::make_shared<TDragonLairRankingData>(dwGuildID, pszGuildName, bMemberCount, dwTime));
}
#endif
...
#if defined(ENABLE_GUILD_DRAGONLAIR_SYSTEM)
	m_DragonLairRankingMap.clear();
	ZeroMemory(&m_bCheckDragonLairRanking, sizeof(m_bCheckDragonLairRanking));
#endif
}
...
#if defined(ENABLE_GUILD_DRAGONLAIR_SYSTEM)
PyObject* guildGetDragonLairRanking(PyObject* poSelf, PyObject* poArgs)
{
	BYTE bType;
	...
	DWORD dwIndex;
	if (!PyTuple_GetUnsignedLong(poArgs, 1, &dwIndex))
		return Py_BadArgument();

	const CPythonGuild::SDragonLairRankingData* pRankingData = CPythonGuild::Instance().GetDragonLairRanking(bType, dwIndex);
	if (pRankingData == nullptr)
		return Py_BuildException();

	int iMinutes = pRankingData->dwTime / 60;
	int iSeconds = pRankingData->dwTime % 60;

	return Py_BuildValue("siii", pRankingData->strGuildName.c_str(), pRankingData->bMemberCount, iMinutes, iSeconds);
}

PyObject* guildGetMyDragonLairRanking(PyObject* poSelf, PyObject* poArgs)
{
	BYTE bType;
	if (!PyTuple_GetByte(poArgs, 0, &bType))
		return Py_BadArgument();
	...
	DWORD dwIndex = 0;
	const CPythonGuild::SDragonLairRankingData* pRankingData = CPythonGuild::Instance().GetMyDragonLairRanking(bType, dwIndex);
	if (pRankingData == nullptr)
		return Py_BuildException();
	...
	return Py_BuildValue("iiii", dwIndex, pRankingData->bMemberCount, iMinutes, iSeconds);
}

PyObject* guildGetDragonLairRankingShowCount(PyObject* poSelf, PyObject* poArgs)
{
	BYTE bType;
	if (!PyTuple_GetByte(poArgs, 0, &bType))
		return Py_BuildException();

	return Py_BuildValue("i", CPythonGuild::Instance().GetDragonLairRankingShowCount(bType));
}

PyObject* guildCheckDragonLairRanking(PyObject* poSelf, PyObject* poArgs)
{
	BYTE bType;
	if (!PyTuple_GetByte(poArgs, 0, &bType))
		return Py_BadArgument();

	return Py_BuildValue("i", CPythonGuild::Instance().CheckDragonLairRanking(bType));
}

PyObject* guildIsDragonLairClearGuild(PyObject* poSelf, PyObject* poArgs)
{
	BYTE bType;
	if (!PyTuple_GetByte(poArgs, 0, &bType))
		return Py_BuildException();

	return Py_BuildValue("b", CPythonGuild::Instance().IsDragonLairClearGuild(bType));
}

PyObject* guildClearDragonLairRanking(PyObject* poSelf, PyObject* poArgs)
{
	BYTE bType;
	if (!PyTuple_GetByte(poArgs, 0, &bType))
		return Py_BadArgument();

	CPythonGuild::Instance().ClearDragonLairRanking(bType);
	return Py_BuildNone();
}
#endif
...
#if defined(ENABLE_GUILD_DRAGONLAIR_SYSTEM)
		// Dragon Lair Ranking
		{ "GetDragonLairRanking", guildGetDragonLairRanking, METH_VARARGS },
		{ "GetMyDragonLairRanking", guildGetMyDragonLairRanking, METH_VARARGS },
		{ "GetDragonLairRankingShowCount", guildGetDragonLairRankingShowCount, METH_VARARGS },
		{ "CheckDragonLairRanking", guildCheckDragonLairRanking, METH_VARARGS },
		{ "IsDragonLairClearGuild", guildIsDragonLairClearGuild, METH_VARARGS },
		{ "ClearDragonLairRanking", guildClearDragonLairRanking, METH_VARARGS },
#endif
...
#if defined(ENABLE_GUILD_DRAGONLAIR_SYSTEM)
	PyModule_AddIntConstant(poModule, "GUILD_DRAGONLAIR_TYPE_RED", GUILD_DRAGONLAIR_TYPE_RED);
	PyModule_AddIntConstant(poModule, "GUILD_DRAGONLAIR_TYPE_BLUE", GUILD_DRAGONLAIR_TYPE_BLUE);
	PyModule_AddIntConstant(poModule, "GUILD_DRAGONLAIR_TYPE_GREEN", GUILD_DRAGONLAIR_TYPE_GREEN);
#endif
}
