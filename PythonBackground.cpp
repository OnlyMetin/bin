...
static const std::string g_strPrivateShopPosEffectName = "d:/ymir work/effect/etc/direction/direction_land2.mse";
#endif
...
#if defined(ENABLE_PRIVATESHOP_SEARCH_SYSTEM)
	// Private Shop Target Effect
	{
		for (const auto& itor : m_kMap_dwTargetID_dwPrivateShopChrID)
		{
			...
			TPixelPosition kPixelPosition;
			pInstance->NEW_GetPixelPosition(&kPixelPosition);
			CreateSpecialEffect(itor.first, +kPixelPosition.x, -kPixelPosition.y, +kPixelPosition.z, g_strPrivateShopPosEffectName.c_str());
		}

		for (auto itor = m_kMap_dwID_kReservePrivateShopPosTargetEffect.begin(); itor != m_kMap_dwID_kReservePrivateShopPosTargetEffect.end();)
		{
			...
			CreateSpecialEffect(itor->first, ilx, ily, fHeight, g_strPrivateShopPosEffectName.c_str());
			itor = m_kMap_dwID_kReservePrivateShopPosTargetEffect.erase(itor);
		}
	}
#endif
}
	...
	switch (m_eShadowTargetLevel)
	{
		...
		case SHADOW_TARGET_ALL:
		{
			...
			if (canRender)
			{
				...
				if (m_eShadowTargetLevel == SHADOW_TARGET_GROUND_AND_SOLO)
...
{
	TMapInfoVector::iterator f = std::find_if(m_kVct_kMapInfo.begin(), m_kVct_kMapInfo.end(), SFindWarpMapName(dwGlobalX, dwGlobalY));
...
void CPythonBackground::Warp(DWORD dwX, DWORD dwY)
{
...
#if defined(ENABLE_PRIVATESHOP_SEARCH_SYSTEM)
	m_kMap_dwTargetID_dwPrivateShopChrID.clear();
	m_kMap_dwID_kReservePrivateShopPosTargetEffect.clear();
#endif

#if defined(ENABLE_FOG_FIX)
	bool bFogMode = CPythonSystem::Instance().GetFogMode();
	if (!m_strMapName.compare("metin2_map_defensewave"))
		bFogMode = true;
...
#if defined(ENABLE_ENVIRONMENT_EFFECT_OPTION)
	if (IsSnowMap(m_strMapName))
	{
		if (GetXMasShowEvent())
		{
			if (GetSnowModeOption())
				EnableSnowEnvironment();

			SetSnowTexture(GetSnowTextureModeOption());
		}
	}
	else
	{
	...
	if (GetDayMode() != DAY_MODE_LIGHT && GetNightModeOption() && IsBoomMap(m_strMapName))
	{
...
#if defined(ENABLE_ENVIRONMENT_EFFECT_OPTION)
void CPythonBackground::SetXMasShowEvent(bool bMode)
{
	m_bXMasShowEvent = bMode;
}

bool CPythonBackground::GetXMasShowEvent() const
{
	return m_bXMasShowEvent;
}

bool CPythonBackground::IsBoomMap(const std::string& sMapName) const
{
	static const std::vector<std::string> vMapName
	{
		...
		"metin2_map_snake_temple_02",
		"metin2_map_whitedragoncave_01",
		"metin2_map_whitedragoncave_02",
		"metin2_map_whitedragoncave_boss",
		"metin2_map_anglar_dungeon_01",
		"metin2_map_otherworld_01",
		"metin2_map_otherworld_02",
		"metin2_map_otherworld_03",
		"metin2_map_otherworld_04",
		"metin2_map_smhgate_a1",
		"metin2_map_smhgate_b1",
		"metin2_map_smhgate_c1",
		"metin2_map_smhgate_threeway",
		"metin2_map_smhgate_desert",
		"metin2_map_smhgate_flame",
		"metin2_map_smhgate_snow",
		"metin2_map_smhgate_devils",
		"metin2_map_smhgate_dawnmist",
	};

	return std::find(vMapName.begin(), vMapName.end(), sMapName) == vMapName.end();
}

bool CPythonBackground::IsSnowMap(const std::string& sMapName) const
{
	static const std::vector<std::string> vMapName
	{
...
void CPythonBackground::SetNightModeOption(bool bMode)
{
	m_bNightMode = bMode;
}

bool CPythonBackground::GetNightModeOption() const
{
	return m_bNightMode;
}

void CPythonBackground::SetSnowModeOption(bool bMode)
{
	m_bSnowMode = bMode;
}

bool CPythonBackground::GetSnowModeOption() const
{
	return m_bSnowMode;
}

void CPythonBackground::SetSnowTextureModeOption(bool bMode)
{
	m_bSnowTextureMode = bMode;
}

bool CPythonBackground::GetSnowTextureModeOption() const
{
	return m_bSnowTextureMode;
}

int CPythonBackground::GetDayMode() const
{
	return m_iDayMode;
}
#endif
...
#if defined(ENABLE_SUMMER_EVENT_ROULETTE)
void CPythonBackground::SetLateSummerEvent(bool bEnable)
{
#if defined(ENABLE_LATE_SUMMER_EVENT_SKYBOX)
	CMapOutdoor& rkMap = GetMapOutdoorRef();
	if (bEnable)
	{
		RegisterEnvironmentData(DAY_MODE_LIGHT, "d:/ymir work/environment/late_summer.msenv");
		SetEnvironmentData(DAY_MODE_LIGHT);
	}
	else
	{
		RegisterEnvironmentData(DAY_MODE_LIGHT, rkMap.GetEnvironmentDataName().c_str());
		SetEnvironmentData(DAY_MODE_LIGHT);
	...
	m_bLateSummerEvent = bEnable;
}

bool CPythonBackground::GetLateSummerEvent() const
{
	return m_bLateSummerEvent;
}
#endif
...
#if defined(ENABLE_PRIVATESHOP_SEARCH_SYSTEM)
	if (m_kMap_dwID_kReservePrivateShopPosTargetEffect.end() != m_kMap_dwID_kReservePrivateShopPosTargetEffect.find(dwID))
		m_kMap_dwID_kReservePrivateShopPosTargetEffect.erase(dwID);

	if (m_kMap_dwTargetID_dwPrivateShopChrID.end() != m_kMap_dwTargetID_dwPrivateShopChrID.find(dwID))
		m_kMap_dwTargetID_dwPrivateShopChrID.erase(dwID);
#endif
...
{
	m_kMap_dwTargetID_dwPrivateShopChrID.emplace(dwID, dwChrVID);
}

void CPythonBackground::CreatePrivateShopPos(DWORD dwID, long lx, long ly)
{
	if (m_kMap_dwTargetID_dwPrivateShopChrID.end() != m_kMap_dwTargetID_dwPrivateShopChrID.find(dwID))
		return;
	...
	if (0.0f == fHeight)
	{
		...
		m_kMap_dwID_kReservePrivateShopPosTargetEffect.emplace(dwID, ReserveTargetEffect);
		return;
	}

	CreateSpecialEffect(dwID, ilx, ily, fHeight, g_strPrivateShopPosEffectName.c_str());
}

void CPythonBackground::DeletePrivateShopPos(DWORD dwID)
{
	DeleteTargetEffect(dwID);
}
#endif

#if defined(ENABLE_CONQUEROR_LEVEL)
bool CPythonBackground::HasSungMaAttr()
{
	CMapOutdoor& rkMap = GetMapOutdoorRef();
	return rkMap.HasSungMaAttr();
}

const CMapBase::SSungmaAttr& CPythonBackground::GetSungMaAttrData()
{
	CMapOutdoor& rkMap = GetMapOutdoorRef();
	return rkMap.GetSungMaAttrData();
}
#endif
