...
public:
	CMainPacketHeaderMap()
	{
		...
		Set(HEADER_GC_ITEM_SET_EMPTY, CNetworkPacketHeaderMap::TPacketType(sizeof(TPacketGCItemSetEmpty), STATIC_SIZE_PACKET));
		...
		Set(HEADER_GC_EMOTE, CNetworkPacketHeaderMap::TPacketType(sizeof(TPacketGCEmote), STATIC_SIZE_PACKET));
		...
		Set(HEADER_GC_PARTY_UNLINK, CNetworkPacketHeaderMap::TPacketType(sizeof(TPacketGCPartyLink), STATIC_SIZE_PACKET));
		...
		Set(HEADER_GC_ACCE_REFINE, CNetworkPacketHeaderMap::TPacketType(sizeof(TPacketGCAcceRefine), DYNAMIC_SIZE_PACKET));
#endif
#if defined(ENABLE_AURA_COSTUME_SYSTEM)
		Set(HEADER_GC_AURA_REFINE, CNetworkPacketHeaderMap::TPacketType(sizeof(TPacketGCAuraRefine), DYNAMIC_SIZE_PACKET));
#endif
#if defined(ENABLE_CHANGE_LOOK_SYSTEM)
		Set(HEADER_GC_CHANGE_LOOK_SET, CNetworkPacketHeaderMap::TPacketType(sizeof(TPacketGCChangeLookSet), STATIC_SIZE_PACKET));
		Set(HEADER_GC_CHANGE_LOOK_DEL, CNetworkPacketHeaderMap::TPacketType(sizeof(TPacketGCChangeLookDel), STATIC_SIZE_PACKET));
		Set(HEADER_GC_CHANGE_LOOK_FREE_SET, CNetworkPacketHeaderMap::TPacketType(sizeof(TPacketGCChangeLookSet), STATIC_SIZE_PACKET));
		Set(HEADER_GC_CHANGE_LOOK_FREE_DEL, CNetworkPacketHeaderMap::TPacketType(sizeof(TPacketGCChangeLookDel), STATIC_SIZE_PACKET));
#endif
		...
		Set(HEADER_GC_PRIVATESHOP_SEARCH, CNetworkPacketHeaderMap::TPacketType(sizeof(TPacketGCPrivateShopSearch), DYNAMIC_SIZE_PACKET));
		Set(HEADER_GC_PRIVATESHOP_SEARCH_OPEN, CNetworkPacketHeaderMap::TPacketType(sizeof(TPacketGCPrivateShopSearchOpen), STATIC_SIZE_PACKET));
#endif
		...
		Set(HEADER_GC_RANKING, CNetworkPacketHeaderMap::TPacketType(sizeof(TPacketGCRanking), DYNAMIC_SIZE_PACKET));
#endif
		...
		Set(HEADER_GC_LOOT_FILTER, CNetworkPacketHeaderMap::TPacketType(sizeof(TPacketGCLootFilter), STATIC_SIZE_PACKET));
#endif
		...
		Set(HEADER_GC_FISHING_GAME, CNetworkPacketHeaderMap::TPacketType(sizeof(TPacketGCFishingGame), STATIC_SIZE_PACKET));
#endif
#if defined(ENABLE_EXTEND_INVEN_SYSTEM)
		Set(HEADER_GC_EXTEND_INVEN, CNetworkPacketHeaderMap::TPacketType(sizeof(TPacketGCExtendInven), STATIC_SIZE_PACKET));
		Set(HEADER_GC_EXTEND_INVEN_ITEM_USE, CNetworkPacketHeaderMap::TPacketType(sizeof(TPacketGCExtendInvenItemUse), STATIC_SIZE_PACKET));
#endif
		...
		Set(HEADER_GC_CUBE, CNetworkPacketHeaderMap::TPacketType(sizeof(TPacketGCCube), STATIC_SIZE_PACKET));
#endif
#if defined(ENABLE_SNOWFLAKE_STICK_EVENT)
		Set(HEADER_GC_SNOWFLAKE_STICK_EVENT, CNetworkPacketHeaderMap::TPacketType(sizeof(TPacketGCSnowflakeStickEvent), STATIC_SIZE_PACKET));
#endif
		...
		Set(HEADER_GC_GUILD_DRAGONLAIR, CNetworkPacketHeaderMap::TPacketType(sizeof(TPacketGCGuildDragonLair), DYNAMIC_SIZE_PACKET));
#endif
#if defined(ENABLE_SUMMER_EVENT_ROULETTE)
		Set(HEADER_GC_MINI_GAME_ROULETTE, CNetworkPacketHeaderMap::TPacketType(sizeof(TPacketGCMiniGameRoulette), STATIC_SIZE_PACKET));
#endif
...
#endif
		// Dodaj przed koñcem konstruktora CMainPacketHeaderMap
		Set(HEADER_GC_DAMAGE_RANKING_DATA, CNetworkPacketHeaderMap::TPacketType(sizeof(TPacketGCDamageRankingData), DYNAMIC_SIZE_PACKET));
	}
};
...
void CPythonNetworkStream::ExitGame()
{
	...
	{
		SendCommandPacket("/phase_select");
	}
}

#if defined(ENABLE_LOCALE_CLIENT)
void CPythonNetworkStream::ExitGameLanguageChange()
{
	if (__IsNotPing())
	{
		LogOutGame();
	...
	{
		SendCommandPacket("/language_change");
	}
}
#endif
...
void CPythonNetworkStream::SetPhaseWindow(UINT ePhaseWnd, PyObject* poPhaseWnd)
{
	...
	if (ePhaseWnd == PHASE_WINDOW_EMPIRE)
		m_bEmpireSelect = false;
#endif
...
UINT CPythonNetworkStream::GetAccountCharacterSlotDatau(UINT iSlot, UINT eType)
{
	...
	switch (eType)
	{
		...
		case ACCOUNT_CHARACTER_SLOT_FORM:
			...
			return rkSimplePlayerInfo.dwMainPart;
		...
		case ACCOUNT_CHARACTER_SLOT_HAIR:
			return rkSimplePlayerInfo.dwHairPart;
#if defined(ENABLE_ACCE_COSTUME_SYSTEM)
		...
		case ACCOUNT_CHARACTER_SLOT_SUNGMA_STR:
			return rkSimplePlayerInfo.bySungmaStr;
		...
		case ACCOUNT_CHARACTER_SLOT_SUNGMA_IMMUNE:
			return rkSimplePlayerInfo.bySungmaImmune;
#endif
	}
	return 0;
}
...
bool CPythonNetworkStream::RecvPhasePacket()
{
	...
	switch (packet_phase.phase)
	{
		...
		case PHASE_GAME: // °ÔÀÓ È­¸é
			SetGamePhase();
#if defined(ENABLE_DISCORD_RPC)
			Discord_Update(true /* bGameState */);
#endif
			break;
...
{
	if (eEmoticon < m_EmoticonTypeVector.size())
	{
		char szEmoticon[255];
		snprintf(szEmoticon, sizeof(szEmoticon), "/emoticon %d", m_EmoticonTypeVector[eEmoticon]);
		SendCommandPacket(szEmoticon);
	}
	else
...
std::string CPythonNetworkStream::GetLoginId()
{
	return m_stID;
}

bool CPythonNetworkStream::SendDamageRankingRequestPacket(DWORD vid)
{
	TPacketCGDamageRankingRequest packet;
	packet.header = HEADER_CG_DAMAGE_RANKING_REQUEST;
	packet.vid = vid;
...
bool CPythonNetworkStream::RecvGCAnimatedDrop()
{
	TPacketGCAnimatedDrop kPacket;
	if (!Recv(sizeof(kPacket), &kPacket))
		return false;

	CPythonItem::Instance().CreateAnimatedDrop(kPacket.vnum, (float)kPacket.x, (float)kPacket.y, (float)kPacket.z);
	return true;
}


bool CPythonNetworkStream::RecvDamageRankingPacket()
{
	TPacketGCDamageRankingData packet;
	if (!Recv(sizeof(packet), &packet))
		return false;

	std::vector<TDamageRankingInfo> rankingData;
	rankingData.resize(packet.count);

	if (packet.count > 0)
	{
		if (!Recv(sizeof(TDamageRankingInfo) * packet.count, &rankingData[0]))
			return false;
	}

	// Budowanie listy Python
	PyObject* pyRankingList = PyList_New(rankingData.size());

	for (size_t i = 0; i < rankingData.size(); ++i)
	{
		PyObject* tuple = PyTuple_New(3);
		PyTuple_SetItem(tuple, 0, PyString_FromString(rankingData[i].name));
		PyTuple_SetItem(tuple, 1, PyLong_FromLong(rankingData[i].damage));
		PyTuple_SetItem(tuple, 2, PyInt_FromLong(rankingData[i].percent));
		PyList_SetItem(pyRankingList, i, tuple);
	}

	// Przeka¿ dane do Pythona
	PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME],
		"RecvDamageRankingPacket",
		Py_BuildValue("(iO)", packet.vid, pyRankingList));

	Py_DECREF(pyRankingList);
