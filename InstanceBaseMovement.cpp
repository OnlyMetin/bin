...
void CInstanceBase::StartWalking()
{
	...
	{
		if (m_adwCRCAffectEffect[AFFECT_GYEONGGONG])
			__DetachEffect(m_adwCRCAffectEffect[AFFECT_GYEONGGONG]);
	...
	{
		if (m_adwCRCAffectEffect[AFFECT_KWAESOK])
			__DetachEffect(m_adwCRCAffectEffect[AFFECT_KWAESOK]);
...
BOOL CInstanceBase::IsActingTargetEmotion()
{
	return m_GraphicThingInstance.IsActTargetEmotion();
}
