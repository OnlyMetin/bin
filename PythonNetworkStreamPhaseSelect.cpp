...
bool CPythonNetworkStream::SendSelectEmpirePacket(DWORD dwEmpireID)
{
	...
	if (m_bEmpireSelect)
	{
		TraceError("SendSelectEmpirePacket - Already selected empire.");
		return false;
	}

	if (dwEmpireID <= CInstanceBase::EMPIRE_NONE || dwEmpireID >= CInstanceBase::EMPIRE_NUM)
		dwEmpireID = GetEmpireID();
#endif
...
bool CPythonNetworkStream::SendSelectCharacterPacket(BYTE index)
{
	...
	strncpy(SelectCharacterPacket.country, LocaleService_GetLocaleName(), COUNTRY_NAME_MAX_LEN);
	SelectCharacterPacket.country[COUNTRY_NAME_MAX_LEN] = '\0';
#endif
...
bool CPythonNetworkStream::__RecvPlayerDestroyFailurePacket()
{
	...
	TPacketGCDestroyCharacterFailure packet;
	if (!Recv(sizeof(TPacketGCDestroyCharacterFailure), &packet))
		return false;

	PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_SELECT], "OnDeleteFailure", Py_BuildValue("(ii)", packet.type, packet.time));
#else
