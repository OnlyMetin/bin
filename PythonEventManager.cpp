...
const long c_lNormal_Waiting_Time = 0; // Quest text delay (old = 10)
...
int CPythonEventManager::GetVisibleLineCount(int iIndex)
{
	if (!CheckEventSetIndex(iIndex))
		return 0;

	TEventSet* pEventSet = m_EventSetVector[iIndex];
	return pEventSet->iVisibleLineCount;
}
...
WORD CPythonEventManager::GetLineHeight(int iIndex)
{
	if (!CheckEventSetIndex(iIndex))
		return 0;
...
void CPythonEventManager::SetFontColor(float iIndex, float fR, float fG, float fB, float fA)
{
	...
	pEventSet->DiffuseColor.r = fR;
	pEventSet->DiffuseColor.g = fG;
	pEventSet->DiffuseColor.b = fB;
	pEventSet->DiffuseColor.a = fA;
}
...
void CPythonEventManager::RefreshLinePosition(TEventSet* pEventSet)
{
	...
	else
	{
		if (GetDefaultCodePage() == CP_1256)
		{
			if (pEventSet->ix && !pEventSet->iWidth)
#if defined(ENABLE_QUEST_WIDTH_EXPANSION)
				pEventSet->iWidth = 660;
#else
				pEventSet->iWidth = 310;
#endif
