...
PyObject* backgroundEnableSnow(PyObject* poSelf, PyObject* poArgs)
{
	...
	if (nIsEnable && rkBG.IsSnowMap(rkBG.GetWarpMapName()))
	{
		if (rkBG.GetSnowModeOption())
			rkBG.EnableSnowEnvironment();
		if (rkBG.GetSnowTextureModeOption())
			rkBG.SetSnowTexture(true);
	}
	else
	{
		rkBG.DisableSnowEnvironment();
		rkBG.SetSnowTexture(false);
	}
#else
	if (nIsEnable)
		rkBG.EnableSnowEnvironment();
...
PyObject* backgroundSetEnvironmentData(PyObject* poSelf, PyObject* poArgs)
{
	...
	if (rkBG.GetEnvironmentData(iIndex, &c_pEnvironmenData) && (iIndex != CPythonBackground::DAY_MODE_DARK || (rkBG.GetNightModeOption() && rkBG.IsBoomMap(rkBG.GetWarpMapName()))))
...
PyObject* backgroundSetFogMode(PyObject* poSelf, PyObject* poArgs)
{
	...
	CPythonBackground::Instance().SetFogMode(bEnable);
	CPythonSystem::Instance().SetFogMode(bEnable);
...
{
	return Py_BuildValue("b", CPythonBackground::Instance().GetFogMode());
}
#endif
...
{
	int iChrVID;
	if (!PyTuple_GetInteger(poArgs, 0, &iChrVID))
		return Py_BadArgument();

	CPythonBackground::Instance().CreatePrivateShopPos(INT32_MAX, iChrVID);
	return Py_BuildNone();
}

PyObject* backgroundDeletePrivateShopPos(PyObject* poSelf, PyObject* poArgs)
{
	CPythonBackground::Instance().DeletePrivateShopPos(INT32_MAX);
	return Py_BuildNone();
}
#endif
...
{
	bool bMode;
	if (!PyTuple_GetBoolean(poArgs, 0, &bMode))
		return Py_BuildException();

	CPythonBackground::Instance().SetXMasShowEvent(bMode);
	return Py_BuildNone();
}
...
{
	CPythonBackground& rkBG = CPythonBackground::Instance();
	return Py_BuildValue("i", rkBG.IsBoomMap(rkBG.GetWarpMapName()));
}
...
PyObject* backgroundSetNightModeOption(PyObject* poSelf, PyObject* poArgs)
{
	...
	CPythonBackground::Instance().SetNightModeOption(bMode);
	return Py_BuildNone();
}
...
PyObject* backgroundSetSnowModeOption(PyObject* poSelf, PyObject* poArgs)
{
	...
	CPythonBackground::Instance().SetSnowModeOption(bMode);
	return Py_BuildNone();
}
...
PyObject* backgroundSetSnowTextureModeOption(PyObject* poSelf, PyObject* poArgs)
{
	...
	CPythonBackground::Instance().SetSnowTextureModeOption(bMode);
	return Py_BuildNone();
}
...
PyObject* backgroundEnableSnowMode(PyObject* poSelf, PyObject* poArgs)
{
	...
	if (bMode && rkBG.GetXMasShowEvent() && rkBG.IsSnowMap(rkBG.GetWarpMapName()))
		rkBG.EnableSnowEnvironment();
	else
		rkBG.DisableSnowEnvironment();
...
PyObject* backgroundEnableSnowTextureMode(PyObject* poSelf, PyObject* poArgs)
{
	...
	if (rkBG.GetXMasShowEvent() && rkBG.GetSnowTextureModeOption() && rkBG.IsSnowMap(rkBG.GetWarpMapName()))
		rkBG.SetSnowTexture(true);
	else
		rkBG.SetSnowTexture(false);
...
PyObject* backgroundSetLateSummerEvent(PyObject* poSelf, PyObject* poArgs)
{
	bool bEnable;
	if (!PyTuple_GetBoolean(poArgs, 0, &bEnable))
		return Py_BuildException();
	...
	CPythonBackground& rkBG = CPythonBackground::Instance();
	rkBG.SetLateSummerEvent(bEnable);

	return Py_BuildNone();
}

PyObject* backgroundGetLateSummerEvent(PyObject* poSelf, PyObject* poArgs)
{
	return Py_BuildValue("b", CPythonBackground::Instance().GetLateSummerEvent());
}
#endif
...
#if defined(ENABLE_SUMMER_EVENT_ROULETTE)
		{ "SetLateSummerEvent", backgroundSetLateSummerEvent, METH_VARARGS },
		{ "GetLateSummerEvent", backgroundGetLateSummerEvent, METH_VARARGS },
#endif
...
#if defined(ENABLE_SHADOW_RENDER_QUALITY_OPTION)
	PyModule_AddIntConstant(poModule, "SHADOW_QUALITY_BAD", CPythonBackground::SHADOW_QUALITY_BAD);
	PyModule_AddIntConstant(poModule, "SHADOW_QUALITY_AVERAGE", CPythonBackground::SHADOW_QUALITY_AVERAGE);
	PyModule_AddIntConstant(poModule, "SHADOW_QUALITY_GOOD", CPythonBackground::SHADOW_QUALITY_GOOD);
	PyModule_AddIntConstant(poModule, "SHADOW_TARGET_NONE", CPythonBackground::SHADOW_TARGET_NONE);
	PyModule_AddIntConstant(poModule, "SHADOW_TARGET_GROUND", CPythonBackground::SHADOW_TARGET_GROUND);
	PyModule_AddIntConstant(poModule, "SHADOW_TARGET_GROUND_AND_SOLO", CPythonBackground::SHADOW_TARGET_GROUND_AND_SOLO);
	PyModule_AddIntConstant(poModule, "SHADOW_TARGET_ALL", CPythonBackground::SHADOW_TARGET_ALL);
#endif
